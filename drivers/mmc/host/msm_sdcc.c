...
#ifdef CONFIG_WIMAX
extern int mmc_wimax_get_status(); 
#endif
...
msmsdcc_pio_irq(int irq, void *dev_id)
{
	...
	if ((host->pio.sg == NULL) || (host->pio.sg->sg_magic != SG_MAGIC)) {
	// if (host->pio.sg == NULL) {
		if (host->pio.sg == NULL) {
		  printk(KERN_INFO "%s: pio scatter list is null - ", mmc_hostname(host->mmc));
		}
	    else if (host->pio.sg->sg_magic != SG_MAGIC) {
		  printk(KERN_INFO "%s: pio scatter sg_magic is not equal - ", mmc_hostname(host->mmc));
		}
