...
if (machine_is_bravoc()) {
	...
	*/
	...
	} else htc_batt_info.rep.battery_full = 0;
...
static void ds2784_battery_algorithm(struct ds2784_device_info *di)
{
	...
	if ((temp > 0) && (temp < 15)) {
		ael_A = 5 * (int)(temp / 5);
		ael_B = temp - ael_A;
		if (ael_A == 0) ael_temp = AEL_0 + ael_B * ((AEL_5 - AEL_0) / 5);
		else if (ael_A == 5) ael_temp = AEL_5 + ael_B * ((AEL_10 - AEL_5) / 5);
		else if (ael_A == 10) ael_temp = AEL_10+ ael_B * ((AEL_15 - AEL_10) /5);
	} else if ((temp <= 0)) ael_temp = AEL_0;
	 else if ((temp >= 15)) ael_temp = AEL_15;

	/*Modify the RARC to MBI base on AEL_TEMP */
	if ((last_level - ael_temp) < 0) htc_batt_info.rep.level = 0;
	else if ((last_level - ael_temp) >= 100) htc_batt_info.rep.level = 100;
	else htc_batt_info.rep.level = (u32)(((last_level - ael_temp) * 100) / (100 - ael_temp));

	/*Don't let MBI drop too fast */
	//pr_info("[HTC_BATT] last MBI = %d, current level(AEL_TEMP) = %d\n",last_update,htc_batt_info.rep.level);
	if (htc_batt_info.rep.level < last_update) {
            if ((last_update >= 5) && (htc_batt_info.rep.level <= (last_update - 5)))
		htc_batt_info.rep.level = last_update - 5;
            else htc_batt_info.rep.level = last_update - 1;
	}
	...
	} else htc_batt_info.rep.full_acr = 0;

	if (htc_batt_info.rep.level >= 100) {
		if ((htc_batt_info.rep.guage_status_reg & 0x80) &&
		    (htc_batt_info.rep.batt_current <= 80))
			htc_batt_info.rep.level = 100;
		else htc_batt_info.rep.level = 99;
	}
